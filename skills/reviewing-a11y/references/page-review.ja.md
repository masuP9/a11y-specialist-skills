...
| 利用可能なツール | 能力 | 制限 |
|-----------------|------|------|
| ブラウザ操作 | アクセシビリティツリー、計算されたrole、視覚的レンダリング | 対応ブラウザ機能またはMCPが必要 |
| Web取得 | HTMLソース、DOM構造 | アクセシビリティツリーなし、JS生成コンテンツ不可 |
| どちらもなし | ページ取得不可 | ユーザーにHTML貼付を依頼 |

### 優先順位

1. **ブラウザ操作**（推奨）: 支援技術が実際に「見る」アクセシビリティツリーを提供
2. **Web取得**: ブラウザ操作が使えない場合のHTML分析フォールバック
...
```
1. 取得: 利用可能なWeb取得機能でURLからHTMLソースを取得
2. HTML構造を直接分析:
...
- **最適な能力を使用**: ブラウザ操作 > Web取得 > ユーザー提供コンテンツ
