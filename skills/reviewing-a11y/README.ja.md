...
このスキルは対象別の参照ガイドを選ぶ**ルーティングパターン**を使用しています。主エージェントがガイドを読み、レビューを直接実行します：
...
## 前提知識（エージェントが既に知っていること）

このスキルは、Claude CodeとCodexが既に以下を知っていることを前提としています：
...
ガイドは主に利用可能な能力を使った**レビュー方法**とワークフローに焦点を当て、エージェントの既存のアクセシビリティ知識を活用します。

人間向けの参考資料として、詳細なWCAGチェックリストが `references/wcag-checklist.ja.md` に用意されています。

## 特徴

- **自動ルーティング**: レビュー対象を特定し、適切な参照ガイドを選択
- **任意の並列化**: ユーザーが明示した場合のみサブエージェントへ分担
...
スキルがURLを検出し、利用可能なブラウザ操作またはWeb取得手段でページレビューガイドを実行します。
...
スキルがファイルパスを検出し、対象と関連コードをコードレビューガイドに従って調査します。
...
スキルがデザイン成果物を検出し、デザインレビューガイドに従って調査します。

## 出力フォーマット

すべての参照ガイドが一貫した出力を生成します：
